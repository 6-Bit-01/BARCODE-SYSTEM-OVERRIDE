...
The built-in `image_gen.imagegen` tool generated one illustration per scene using the inspected current models. `assets/intro/art-manifest.json` preserves each complete prompt, ordered references, original generated filename/hash, dimensions and production-file hash. The original generated PNGs remain unchanged. After #42, the owner requested a targeted anatomical correction to DJ's page-5 knob hand. The built-in image editor corrected its thumb/index pinch, curled fingers and wrist while preserving the scene; the correction prompt and original/new source hashes are appended in the same manifest. This explicit request supersedes the earlier no-regeneration constraint for this one correction. Runtime copies are WebP quality 96 at their original approximately 1862×845 dimensions (about 2.2:1), with no semantic editing or production cropping. The eight assets total approximately 5.2 MiB.

Following the owner's PR #40 missing-image report, CutsceneSystem requests commit-pinned public copies of these exact assets and attempts the bundled relative path once if needed. INTRO_REPAIR.md and `verification/intro-asset-delivery.json` record delivery and its limits. A single acquired canvas context is reused for all pages and polls. Each full-width illustration has individually placed speech balloons, offscreen comms cards and monitor-aligned readouts, with white for 6 Bit, cyan for DJ, yellow for Cache and red for Mac. Text is rendered over the scene without changing its image pixels. The staged cue clock uses the existing presentation poll. Five readouts are transformed/clipped into monitor glass, and the other captions occupy the page margin; reduced effects use the static endpoint. INTRO_CUE_STAGING.md records their order and timing. No duplicate side crops remain.
