...
The production check covers choosing and replacing a lane before a downbeat, dodging after a choice, one added part per bar, a brief reachable x4 with E/RB, a close cut that charges Zone without silently carrying music, an adjacent pass with steering drift, a paired-gate hit, audio dropout, the whole 100-bar song, Echo exit and old saves. Local logic and CI browser decoding verify mechanics and MP3 paths. They cannot judge whether the timing and sound feel right in merged Makko with a physical controller.
