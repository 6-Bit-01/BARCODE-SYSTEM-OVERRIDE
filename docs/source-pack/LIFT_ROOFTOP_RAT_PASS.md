...
- The actual encounter-4 drone spawns at x=3660 and patrols x=3625–3695, beyond the overhead steps. Its body and descending stomp approach stay clear. The tower climbing platforms retain their route geometry.
- Owner follow-up: move the Firewall high step 170 units left, from x=2330 to x=2160. Its 136-unit landing, illustrated depth, brackets and shadow now clear the cabin's full vertical path; the shadow ends at x=2333, leaving 33 units before the shaft at x=2366. The canopy → step → rooftop route retains the same heights and one-jump movement.
...
3. Power with two beats, ride past the relocated high step to the upper stop and walk left onto the actual rooftop without jumping. Record the clear shaft, aligned floor, successful exit and lift returning while 6 Bit stays on the roof. Repeat after pause/resume, then climb canopy → relocated step → rooftop.
