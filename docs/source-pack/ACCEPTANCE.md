...
Import the exact merged main SHA after CI. Center in one lane for half a second; the header should name that lane and count down to the next bar, and a light mark should appear on the road. Steer away to dodge traffic; the choice should stay, then that one part should join on the bar line and last to the current four-bar section's end. Hold a different lane before the bar line to replace the first choice. Try several lanes in one bar and confirm that only the last one joins. Across separate bars, choose different parts to build a stack; it should take deliberate bar-by-bar choices rather than one sweep. E/RB immediately keeps one lane through the current remainder and next four bars once per section.
