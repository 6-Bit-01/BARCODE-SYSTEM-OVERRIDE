...
The former sparse-track availability claim was corrected after the owner questioned it. All four lanes have aligned song-length recordings and can be captured in any section. Undercurrent plays in verse A; Breakaway is much softer there, not absent. Some intro samples are silent, so a visual lock cannot add sound the recording does not contain. Pressure drums remain steady; clean score scales with the number of captured lanes, up to x4. A single lane cannot duplicate itself into a multiplier. The old automatic 1.5-second eight-bar carry and the 60/40 ink costs are removed. The shared input, audio, RAF, save and pause owners remain.
...
## Review corrections after merged #106

The pause menu now teaches the same free hold/E/RB/Zone controls as the road header. The RB status reflects whether the current recorded remainder, the upcoming four bars, or both actually changed. The old assertion that the next Breakaway/Undercurrent verse A was unavailable was wrong and has been removed; a chorus-to-verse RB capture now promises both correctly. Paired traffic pays clean passes after the whole crossing resolves, discarding any near-miss rewards at a crossing where a vehicle hits; the hit also resets near-miss progress. Focused tests reproduce contact from either blocked lane at the 975 gate, including a neighboring pass processed before the impact. These are clarity and skill-credit corrections to the approved system, with Makko assessment still open.
