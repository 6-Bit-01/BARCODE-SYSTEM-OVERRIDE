...
`intro-01.webp` through `intro-08.webp` and `intro-contact.webp` render the actual CutsceneSystem → IntroSequence path with the existing images. `intro-skip.webp` shows the hold progress; `intro-reduced-effects.webp` shows the static displaced caption and inspection response. Generated by `node tools/render-intro.cjs /path/to/existing-intro-images`. Text/page bounds are asserted. These are native Canvas layout diagnostics, not Makko screenshots or an audible/controller acceptance claim.
