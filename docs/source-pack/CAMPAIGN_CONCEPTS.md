...
Recommended guaranteed key IDs: `stem.voice`, `stem.bass`, `stem.drums`, `stem.synth`, `stem.samples`, `stem.noise_fx`. A level clear grants its key once; keys cannot be missed, lost or consumed. Six keys open the finale. They represent progress and unlock compatible layers of a separate arrangement; never overlay six unrelated level songs.

Recommended optional module IDs: `module.beat_anchor`, `module.phase_capacitor`, `module.breach_emitter`, `module.restore_point`, `module.crew_link`, `module.root_credential`. Names/effects are tunable, but the design purpose is fixed: exploration rewards change finale options without blocking the main route.

| Working module | Candidate finale payoff |
|---|---|
| Beat Anchor | Clarify an audio/decoy puzzle and a readable counter window |
| Phase Capacitor | Transit shortcut and a finale-only evasive option |
| Breach Emitter | Optional armory route and earlier armor break |
| Restore Point | Recovery opportunity or a limited rollback |
