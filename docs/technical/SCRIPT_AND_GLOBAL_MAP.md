...
- Dash and fast-fall are removed from active production controls. Down Arrow is the primary attack, not fast-fall. H starts the existing hacking system when gameplay accepts the action; H is blocked while airborne or while Rhythm Combat Mode is active. Explicit nearby-terminal targeting is deferred to a future scoped implementation.
