...
window.Enemy = class Enemy {
  ...
  getDrawLayer() {
    ...
    if (this.type === 'corrupted') return 0;
    if (this.type === 'firewall') return 2;
    return 1;
  }
};

// ==========================================
// 2. JAMMER ENEMY CLASS (Fixed Audio)
// ==========================================
...
window.JammerEnemy = class JammerEnemy extends window.Enemy {
  ...
  initAudio() {
    if (this.audioInitialized || !this.audioUrl) return;
    
    this.audioElement = new Audio(this.audioUrl);
    this.audioElement.loop = true;
    this.audioElement.volume = 0; // Start silent
    ...
    if (playPromise !== undefined) {
      playPromise.then(_ => {
        ...
        console.log("Jammer audio blocked by browser policy - waiting for interaction");
        this.audioInitialized = false;
      });
    }
  }

  // NEW: Update audio volume based on player distance
  updateProximityAudio(player) {
    // Initialize if needed
    if (!this.audioInitialized && !this.audioElement) {
        this.initAudio();
    }
    
    if (!this.audioElement || !player) return;
    ...
    if (dist > this.maxDistance) {
        ...
        // Linear fade in as player gets closer
        const volume = (1 - (dist / this.maxDistance)) * this.baseVolume;
        // Clamp to ensure valid range [0, 1]
        this.audioElement.volume = window.clamp(volume, 0, 1);
        
        // Ensure it's playing if it was paused or failed to start
        if (this.audioElement.paused && volume > 0) {
  ...
  getDrawLayer() {
    if (this.type === 'jammer') return -2; // Draw behind everything including player
