  ...
  const menu = BARCODE.PauseMenu = {
    ...
    draw(ctx) {
      ...
      const controls = road ? (BARCODE.GamepadUI?.connected
        ? ['Stick / D-pad: Steer; Up: Faster; Down: Brake', `${BARCODE.ControllerSettings.button(4)}: Turbo; ${BARCODE.ControllerSettings.button(5)}: Buffer Echo`, 'Face buttons: Surge / Push / Brace / Refill', 'Drive into the glowing SAFE pulse and tap its button on the beat.', 'A catch raises that lane for 4 bars; consecutive catches hold 8.', `${BARCODE.ControllerSettings.button(9)}: Pause / Settings`]
        : ['A / D or Left / Right: Steer; Up / W: Faster; Down / S: Brake', 'Space: Turbo; H: Buffer Echo', 'K: Surge; L: Push; J: Brace; I: Refill', 'Drive into the glowing SAFE pulse and tap its key on the beat.', 'A catch raises that lane for 4 bars; consecutive catches hold 8.', 'P: Pause'])
