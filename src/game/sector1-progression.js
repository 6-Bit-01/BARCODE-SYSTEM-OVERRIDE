...
(function() {
  ...
  window.Sector1Progression = class Sector1Progression {
    ...
    updateEncounter() { const index = ENCOUNTERS.findIndex(e => e.id === this.state); const def = ENCOUNTERS[index]; if (!def) return; const px = this.player?.position?.x || 0; if (!this.spawnedEncounterIds.has(def.id) && px >= def.triggerX) this.spawnEncounter(def); const survivors = this.activeEncounterEnemies.filter(e => e && e.active && !e._defeatRecorded).length; if (this.activeEncounterId === def.id && this.activeEncounterPacket === 0 && (!this.pendingSpawns || this.pendingSpawns.length === 0) && survivors <= 1) { this.packetGraceMs = Number.isFinite(this.packetGraceMs) ? this.packetGraceMs : 900; } if (this.packetGraceMs > 0 && !window.hackingSystem?.isActive?.()) { this.packetGraceMs -= 16; if (this.packetGraceMs <= 0) this.releaseNextPacket(def); } const noPendingSpawns = !this.pendingSpawns || this.pendingSpawns.length === 0; if (this.activeEncounterId === def.id && noPendingSpawns && this.activeEncounterEnemies.length > 0 && this.activeEncounterEnemies.every(e => !e.active || e._defeatRecorded)) { this.openEncounterGate(def.id); if (index < ENCOUNTERS.length - 1) { this.state = ENCOUNTERS[index + 1].id; this.activeEncounterId = null; this.activeEncounterEnemies = []; this.closedGateEncounterId = null; this.prepareAssetsForEncounter(index + 1); } } }
    spawnEncounter(def) { this.spawnedEncounterIds.add(def.id); this.activeEncounterId = def.id; this.closedGateEncounterId = def.id; this.activeEncounterEnemies = []; this.activeEncounterPacket = 0; this.packetGraceMs = 0; this.pendingSpawns = (def.packets?.[0] || def.enemies).map((spec, i) => ({ spec, encounterId: def.id, index: i, delayMs: i * SPAWN.staggerMs })); }
    releaseNextPacket(def) { this.activeEncounterPacket = 1; this.pendingSpawns = (def.packets?.[1] || []).map((spec, i) => ({ spec, encounterId: def.id, index: (def.packets?.[0]?.length || 0) + i, delayMs: i * SPAWN.staggerMs })); }
    ...
    updateSignalLift(deltaTime) { if (!this.signalLift) this.resetSignalLift(); const l = this.signalLift; l.prevY = l.y; if (this.isGameplaySuppressed()) return; const player = this.player || window.player; const supported = player && player.supportedSurfaceId === SIGNAL_LIFT.id; if (l.state === 'charged') l.state = 'moving'; if (l.state === 'moving') l.y = Math.max(SIGNAL_LIFT.topY, l.y - SIGNAL_LIFT.speed * deltaTime / 1000); else if (l.state === 'returning') l.y = Math.min(SIGNAL_LIFT.bottomY, l.y + SIGNAL_LIFT.speed * deltaTime / 1000); if (l.state === 'moving' && l.y <= SIGNAL_LIFT.topY) l.state = 'dormant'; if (l.state === 'returning' && l.y >= SIGNAL_LIFT.bottomY) { l.state = 'dormant'; l.charges = 0; } if (!supported && l.y <= SIGNAL_LIFT.topY + 1) { l.returnTimerMs = (l.returnTimerMs || SIGNAL_LIFT.returnDelayMs) - deltaTime; if (l.returnTimerMs <= 0) l.state = 'returning'; } else l.returnTimerMs = SIGNAL_LIFT.returnDelayMs; const dy = l.y - l.prevY; if (supported && dy) player.position.y += dy; }
    resetSignalLift() { this.signalLift = { ...SIGNAL_LIFT, y: SIGNAL_LIFT.bottomY, prevY: SIGNAL_LIFT.bottomY, state: 'dormant', charges: 0, returnTimerMs: SIGNAL_LIFT.returnDelayMs }; }
    chargeSignalLift() { if (!this.signalLift) this.resetSignalLift(); const player = this.player || window.player; if (!player || player.supportedSurfaceId !== SIGNAL_LIFT.id) return { ok:false, reason:'not-supported' }; this.signalLift.charges = Math.min(SIGNAL_LIFT.requiredCharges, this.signalLift.charges + 1); this.signalLift.state = this.signalLift.charges >= SIGNAL_LIFT.requiredCharges ? 'charged' : 'charging'; return { ok:true, charges:this.signalLift.charges, state:this.signalLift.state }; }
    ...
    applyPlayerStageCollision(player, movement = {}) { if (!player || !player.velocity || player.velocity.y < 0) return false; const previousAnchorY = Number.isFinite(movement.previousFootY) ? movement.previousFootY : player.position.y; const currentAnchorY = Number.isFinite(movement.currentFootY) ? movement.currentFootY : player.position.y; const previousVisualFootY = previousAnchorY + PLAYER_VISUAL_FOOT_OFFSET; const currentVisualFootY = currentAnchorY + PLAYER_VISUAL_FOOT_OFFSET; const previousX = Number.isFinite(movement.previousX) ? movement.previousX : player.position.x; const footHalfWidth = 18; const verticalTravel = currentVisualFootY - previousVisualFootY; let landing = null; const moving = this.signalLift ? [{ id: SIGNAL_LIFT.id, x: this.signalLift.x, y: this.signalLift.y, w: this.signalLift.w, h: this.signalLift.h, moving: true }] : []; for (const surface of STAGE_SURFACES.concat(moving)) { if (previousVisualFootY > surface.y || currentVisualFootY < surface.y) continue; const crossingT = verticalTravel > 0 ? Math.max(0, Math.min(1, (surface.y - previousVisualFootY) / verticalTravel)) : 1; const crossingX = previousX + (player.position.x - previousX) * crossingT; const overlapsX = crossingX + footHalfWidth > surface.x && crossingX - footHalfWidth < surface.x + surface.w; if (overlapsX && (!landing || crossingT < landing.crossingT)) landing = { surface, crossingT }; } if (!landing) return false; player.position.y = landing.surface.y - PLAYER_VISUAL_FOOT_OFFSET; player.velocity.y = 0; player.grounded = true; player.supportedSurfaceId = landing.surface.id; return true; }
    ...
    debugResetSignalLift() { if (!debugAllowed()) return debugDisabled(); this.resetSignalLift(); return this.getDiagnostics(); }
    debugChargeSignalLift() { if (!debugAllowed()) return debugDisabled(); if (this.player) this.player.supportedSurfaceId = SIGNAL_LIFT.id; return this.chargeSignalLift(); }
