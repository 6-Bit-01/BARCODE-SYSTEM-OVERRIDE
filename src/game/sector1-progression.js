...
(function() {
  ...
  const SIGNAL_LIFT = Object.freeze({ id: 'signal-lift', x: 660, w: 132, h: 10, bottomY: GROUND_Y + PLAYER_VISUAL_FOOT_OFFSET, topY: 492, speed: 220, returnDelayMs: 900, requiredCharges: 2 });
  ...
  window.Sector1Progression = class Sector1Progression {
    ...
    update(deltaTime = 0) {
      ...
      this.updateSignalLift(deltaTime); this.updateSignalAmp();
      ...
      if (/^encounter_/.test(this.state)) { this.applyGateCollision(); this.updateEncounter(deltaTime); this.updatePendingSpawns(deltaTime); }
    ...
    updateEncounterPackets(def, deltaTime = 0) { const packets = def.packets || [def.enemies || []]; if (this.activeEncounterPacket >= packets.length - 1) return; const survivors = this.activeEncounterEnemies.filter(e => e && e.active && !e._defeatRecorded).length; const noPendingSpawns = !this.pendingSpawns || this.pendingSpawns.length === 0; if (noPendingSpawns && survivors <= 1 && this.packetGraceMs === null) this.packetGraceMs = 900; if (this.packetGraceMs !== null && !window.hackingSystem?.isActive?.()) { this.packetGraceMs = Math.max(0, this.packetGraceMs - deltaTime); if (this.packetGraceMs <= 0) this.releaseNextPacket(def); } }
    ...
    drawSignalLift(ctx) { if (!ctx || !this.signalLift) return; const l = this.signalLift; ctx.save(); ctx.shadowColor = l.state === 'charging' ? '#ff00ff' : '#00ffff'; ctx.shadowBlur = 16; ctx.fillStyle = l.charges >= SIGNAL_LIFT.requiredCharges ? 'rgba(255,0,255,0.8)' : 'rgba(0,255,255,0.72)'; ctx.fillRect(l.x, l.y - 6, l.w, 12); ctx.strokeStyle = '#ffffff'; ctx.strokeRect(l.x, l.y - 6, l.w, 12); ctx.restore(); }
    ...
    isPlayerSupportedByLift(player = this.player || window.player) { if (!player || !this.signalLift || !player.grounded || player.supportedSurfaceId !== SIGNAL_LIFT.id) return false; const footY = player.position.y + PLAYER_VISUAL_FOOT_OFFSET; const footHalfWidth = 18; return player.position.x + footHalfWidth > this.signalLift.x && player.position.x - footHalfWidth < this.signalLift.x + this.signalLift.w && Math.abs(footY - this.signalLift.y) <= 4; }
    updateSignalLift(deltaTime) { if (!this.signalLift) this.resetSignalLift(); const l = this.signalLift; l.prevY = l.y; if (this.isGameplaySuppressed()) return; const player = this.player || window.player; let supported = this.isPlayerSupportedByLift(player); if (player && player.supportedSurfaceId === SIGNAL_LIFT.id && !supported) player.supportedSurfaceId = null; if (l.state === 'charged') l.state = 'moving'; if (l.state === 'moving') l.y = Math.max(SIGNAL_LIFT.topY, l.y - SIGNAL_LIFT.speed * deltaTime / 1000); else if (l.state === 'returning') l.y = Math.min(SIGNAL_LIFT.bottomY, l.y + SIGNAL_LIFT.speed * deltaTime / 1000); if (l.state === 'moving' && l.y <= SIGNAL_LIFT.topY) l.state = 'dormant'; if (l.state === 'returning' && l.y >= SIGNAL_LIFT.bottomY) { l.state = 'dormant'; l.charges = 0; } if (!supported && l.y <= SIGNAL_LIFT.topY + 1) { l.returnTimerMs = (l.returnTimerMs || SIGNAL_LIFT.returnDelayMs) - deltaTime; if (l.returnTimerMs <= 0) l.state = 'returning'; } else l.returnTimerMs = SIGNAL_LIFT.returnDelayMs; const dy = l.y - l.prevY; if (supported && dy) player.position.y += dy; }
    resetSignalLift() { this.signalLift = { ...SIGNAL_LIFT, y: SIGNAL_LIFT.bottomY, prevY: SIGNAL_LIFT.bottomY, state: 'dormant', charges: 0, returnTimerMs: SIGNAL_LIFT.returnDelayMs }; const player = this.player || window.player; if (player && player.supportedSurfaceId === SIGNAL_LIFT.id) player.supportedSurfaceId = null; }
    chargeSignalLift() { if (!this.signalLift) this.resetSignalLift(); const player = this.player || window.player; if (!this.isPlayerSupportedByLift(player)) return { ok:false, reason:'not-supported' }; this.signalLift.charges = Math.min(SIGNAL_LIFT.requiredCharges, this.signalLift.charges + 1); this.signalLift.state = this.signalLift.charges >= SIGNAL_LIFT.requiredCharges ? 'charged' : 'charging'; return { ok:true, charges:this.signalLift.charges, state:this.signalLift.state }; }
    ...
    applyPlayerStageCollision(player, movement = {}) { if (!player || !player.velocity || player.velocity.y < 0) return false; const previousAnchorY = Number.isFinite(movement.previousFootY) ? movement.previousFootY : player.position.y; const currentAnchorY = Number.isFinite(movement.currentFootY) ? movement.currentFootY : player.position.y; const previousVisualFootY = previousAnchorY + PLAYER_VISUAL_FOOT_OFFSET; const currentVisualFootY = currentAnchorY + PLAYER_VISUAL_FOOT_OFFSET; const previousX = Number.isFinite(movement.previousX) ? movement.previousX : player.position.x; const footHalfWidth = 18; const verticalTravel = currentVisualFootY - previousVisualFootY; let landing = null; const moving = this.signalLift ? [{ id: SIGNAL_LIFT.id, x: this.signalLift.x, previousY: this.signalLift.prevY, y: this.signalLift.y, w: this.signalLift.w, h: this.signalLift.h, moving: true }] : []; for (const surface of STAGE_SURFACES.concat(moving)) { const surfacePrevY = Number.isFinite(surface.previousY) ? surface.previousY : surface.y; if (previousVisualFootY > surfacePrevY || currentVisualFootY < surface.y) continue; const crossingT = verticalTravel > 0 ? Math.max(0, Math.min(1, (surface.y - previousVisualFootY) / verticalTravel)) : 1; const crossingX = previousX + (player.position.x - previousX) * crossingT; const overlapsX = crossingX + footHalfWidth > surface.x && crossingX - footHalfWidth < surface.x + surface.w; if (overlapsX && (!landing || crossingT < landing.crossingT)) landing = { surface, crossingT }; } if (!landing) return false; player.position.y = landing.surface.y - PLAYER_VISUAL_FOOT_OFFSET; player.velocity.y = 0; player.grounded = true; player.supportedSurfaceId = landing.surface.id; return true; }
    ...
    debugResetSignalLift() { if (!debugAllowed()) return debugDisabled(); this.resetSignalLift(); if (this.player) { this.player.position.x = SIGNAL_LIFT.x + SIGNAL_LIFT.w / 2; this.player.position.y = SIGNAL_LIFT.bottomY - PLAYER_VISUAL_FOOT_OFFSET; this.player.velocity.x = 0; this.player.velocity.y = 0; this.player.grounded = true; this.player.supportedSurfaceId = SIGNAL_LIFT.id; } return this.getDiagnostics(); }
