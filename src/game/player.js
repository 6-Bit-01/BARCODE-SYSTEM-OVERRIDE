...
window.Player = class Player {
  constructor(x, y) {
    ...
    this.width = 70;  // Stable gameplay body width
    this.height = 156; // Stable gameplay body height
  ...
  update(deltaTime, allowMovement = true) {
    try {
      ...
      // Ground collision uses the authoritative Level 1 foot line.
  ...
  getGameplayBody() {
    return window.BARCODE?.Level01Presentation?.playerBody ? window.BARCODE.Level01Presentation.playerBody(this) : { x: this.position.x - this.width / 2, y: this.position.y - this.height, w: this.width, h: this.height, left: this.position.x - this.width / 2, right: this.position.x + this.width / 2, top: this.position.y - this.height, bottom: this.position.y };
  }

  getVisualBounds() {
    const t = window.BARCODE?.Level01Presentation?.playerTransform ? window.BARCODE.Level01Presentation.playerTransform(this) : null;
    return t || this.getGameplayBody();
  }
  ...
  drawHitbox(ctx) {
    if (!ctx || !window.DEBUG_SHOW_HITBOXES) return;
    const b = this.getGameplayBody();
    ctx.save();
  ...
  drawSprite(ctx) {
    ...
    // Visual presentation is bottom-centered by Level01Presentation.
    ...
    const animationScale = visual.scale || 1.5;
    this.sprite.draw(ctx, drawX, drawY, {
      scale: animationScale, // Normalized Level 1 target height
