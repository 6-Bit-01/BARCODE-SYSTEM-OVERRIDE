...
(function(B) {
  ...
  class MusicDirector {
    ...
    reset() {
      ...
      this.comboMilestone = 0; this.graph = null; this.audio = null;
    }
    accent(kind) {
    ...
    bindColour(audio, source) {
      ...
      try {
        ...
        send.gain.value = 0; feedback.gain.value = 0.12; wet.gain.value = 0.18;
        ...
        this.graph = { gain, filter, send, delay, nodes, output: audio.musicGain };
        return this.graph;
      } catch (_) {
    ...
    apply(audio) {
      ...
      const phrase = grid ? Math.floor(grid.beatIndex / (grid.phraseBeatCount || grid.beatsPerBar)) : null;
      if (!generationChanged && phrase !== null && phrase !== this.lastPhrase) this.variation = (this.variation + 1) % 4;
      ...
      const baseline = enabled ? levels : mix.states[window.rhythmSystem?.isActive?.() || window.hackingSystem?.isActive?.() ? ((window.enemyManager?.getActiveEnemies?.().length || 0) > 0 ? 'legacyRhythm' : 'legacyRhythmSolo') :
        (window.enemyManager?.getActiveEnemies?.().length || 0) > 0 ? 'legacyCombat' : 'explore'] || fallback;
      const variant = enabled && !['bossFinal', 'victory', 'counter'].includes(this.state) ? [1, 0.82, 0.94, 0.88][this.variation] : 1;
      for (const source of profile.arrangement.sources) {
        ...
        const volume = cutscene ? 0 : clamp(level * (role === mix.colourRole ? variant : 1), 0, 0.8);
      ...
      if (!enabled || cutscene) { this.disposeGraph(); this.pendingAccent = null; }
        ...
        const graph = this.bindColour(audio, colour);
        if (graph && (beatChanged || !grid || generationChanged)) {
          ...
          const cutoff = this.state === 'hack' ? 4200 : this.state === 'bossFinal' ? 16000 : 6500 + Math.min(8000, combo * 350);
          ...
          if (grid && this.pendingAccent && !generationChanged && grid.beatIndex - this.lastAccentBeat >= 4) {
            ...
            this.ramp(graph.send.gain, 0.55, now, 0.02);
    ...
    diagnostics() { return { profileId: this.profileId, state: this.state, pending: this.pending, variation: this.variation, volumes: { ...this.volumes }, effects: !!this.graph }; }
  }
