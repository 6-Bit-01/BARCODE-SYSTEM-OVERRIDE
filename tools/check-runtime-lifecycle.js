...
for (const file of fs.readdirSync(path.join(root, 'tools')).filter(f => f.endsWith('.js') && f !== 'check-runtime-lifecycle.js' && f !== 'check-action-combat.js' && f !== 'check-level-01-mission.js' && f !== 'check-level-01-gameplay-dynamics.js')) {
