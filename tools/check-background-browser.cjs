...
async function main() {
  ...
  const rendered=await evaluate(`(async()=>{
    ...
    let upper=0,lower=0; for(let y=0;y<1080;y++)for(let x=0;x<1920;x+=3){const i=(y*1920+x)*4; const d=Math.abs(frames[0][i]-frames[1][i]);if(y<140)upper+=d;else if(y>220)lower+=d;}
