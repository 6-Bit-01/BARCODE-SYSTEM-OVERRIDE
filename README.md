...
The actual opening correction now builds on merged PR #39: eight comic pages with new scene artwork based on the supplied character models, independent S/B skip holds, and a direct tutorial-to-mission handoff. Read [the model and artwork guide](docs/source-pack/INTRO_ART_DIRECTION.md), [the complete opening and previews](docs/source-pack/INTRO_OVERHAUL.md), [current state](docs/source-pack/CURRENT_STATE.md) and [acceptance route](docs/source-pack/ACCEPTANCE.md). Live Makko acceptance is pending.
